...
func Run(noOfAllRequests int, lengthOfEachRequest int, allRequests []float64, Eps float64, MinSamples float64, autoGenerateSampleData bool, appendName string) {
	...
	err = PlotResultsPCA(noOfAllRequests, proj)
	if err != nil {
		log.Fatalf("\n%+v", err)
...
func PlotResultsPCA(noOfRequests int, X *mat.Dense) error {
	now := time.Now()
	// Save the plot to a PNG file.
	pngfile := "komu" + "_" + now.Format("Jan_2_2006_15_04_05") + ".png"
	...
	pt.Add(plotter.NewGrid())
	pt.Title.Text = fmt.Sprint("test standalone mat.Dense plot")
